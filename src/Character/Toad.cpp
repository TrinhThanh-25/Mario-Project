...
Toad::Toad(ModePlayer mode, Vector2 pos, Vector2 vel, Color color, float speedX, float maxSpeedX, float acceleration, float friction, float floatTime, float jumpSpeed, int initialLives) :
    Character(CharacterName::TOAD, mode, pos, {32, 40}, vel, color, speedX, maxSpeedX, acceleration, friction, floatTime, jumpSpeed, initialLives) {
    name = "Toad";
}
...
void Toad::transitionToFlower() {
    type = CharacterType::FLOWER;
    size = {34, 54};
    maxFrame = 3;
}

json Toad::saveToJson() const {
    json j = Character::saveToJson();
    return j;
}

void Toad::loadFromJson(const json& j) {
